 ...
 * port and bit-mask arguments libopencm3 expects. GPIO port bases are
 * 256-byte aligned so the bit number lives in the bottom nibble without
 * collision. */
static inline uint32_t pin_port(uint32_t pin) { return pin & 0xffffff00U; }
static inline uint16_t pin_mask(uint32_t pin) { return 1U << (pin & 0xfU); }
...
#define PIN_TO_CHANNEL(pin) (pin_port(pin) == GPIOA \
				? (pin & 0xfU) \
				: ((pin & 0xfU) + 8))


/* Helper macros wrap the libopencm3 calls so the do/while lets them
 * expand as statements anywhere a function call would be valid. */

#define pinMode(pin, mode) \
	do { \
		gpio_mode_setup(pin_port(pin), (mode), GPIO_PUPD_NONE, pin_mask(pin)); \
		gpio_set_output_options(pin_port(pin), GPIO_OTYPE_PP, \
				GPIO_OSPEED_MED, pin_mask(pin)); \
	} while (0)

#define pinModePull(pin, mode, pull) \
	do { \
		gpio_mode_setup(pin_port(pin), (mode), (pull), pin_mask(pin)); \
		gpio_set_output_options(pin_port(pin), GPIO_OTYPE_PP, \
				GPIO_OSPEED_MED, pin_mask(pin)); \
	} while (0)
...
#define pinModeAF(pin, AF, pullUpDown, speed) \
	do { \
		gpio_mode_setup(pin_port(pin), GPIO_MODE_AF, (pullUpDown), pin_mask(pin)); \
		gpio_set_output_options(pin_port(pin), GPIO_OTYPE_PP, \
				(speed), pin_mask(pin)); \
		gpio_set_af(pin_port(pin), AF(pin), pin_mask(pin)); \
	} while (0)
...
#define digitalWrite(pin, val) \
	do { \
		if (val) { \
			gpio_set(pin_port(pin), pin_mask(pin)); \
		} else { \
			gpio_clear(pin_port(pin), pin_mask(pin)); \
		} \
	} while (0)

#define digitalRead(pin) \
	(gpio_get(pin_port(pin), pin_mask(pin)) ? 1 : 0)
